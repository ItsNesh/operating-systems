...
That “negative value equals waiters” invariant applies to **abstract textbook** semaphores (Dijkstra-style), not to the POSIX API you use in C. Keep the conceptual model for proofs, but don’t rely on it in code. :contentReference[oaicite:1]{index=1}
...
- **Multiple TLBs** — In practice, CPUs use **separate iTLB/dTLB** and sometimes **L1/L2 TLBs** for hierarchy/latency—not a “per-process TLB.” Per-process TLBs are **not** how general-purpose CPUs handle context switches; they use **flushes** and/or **ASIDs** instead. :contentReference[oaicite:3]{index=3}
...
> “**How does a dirty bit help optimize TLB management?**” :contentReference[oaicite:4]{index=4}

### ✅ Revised
**Better wording:** “How does a **dirty bit** help optimize **page replacement / write-back** decisions?”  
*Reason*: The dirty (modified) bit primarily informs whether a **page** must be written back to disk on eviction. While TLB entries may mirror a D bit, the optimization target is **paging I/O**, not TLB management itself. :contentReference[oaicite:5]{index=5}
...
> “**What would happen if we removed `volatile`… Why is `volatile` necessary here?**” (re: `static volatile int counter = 0;`) :contentReference[oaicite:6]{index=6}
...
This example is **correct**; consider adding: “Assumes sequential access with one compulsory miss per distinct page.” :contentReference[oaicite:8]{index=8}
...
- **Understand the invariant (clarified)**: In POSIX, semaphore values are **non-negative**; you **cannot** infer the number of waiters from a negative value. The “negative equals waiters” rule is a **theoretical** model used in proofs, not the POSIX API. :contentReference[oaicite:9]{index=9}
...
| **Separate / Hierarchical TLBs** | iTLB vs dTLB; possibly L1/L2 TLBs | Latency/scalability benefits | Not per-process; still need ASIDs or flushes for switches | :contentReference[oaicite:10]{index=10}

---

### 3) Week 4 – Key Takeaways / Questions (replace the “dirty bit” question)
- **How does a dirty bit help optimize page replacement and write-back?** (*Answer you can add later: it avoids unnecessary disk writes when evicting clean pages.*) :contentReference[oaicite:11]{index=11}
